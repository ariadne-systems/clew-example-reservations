...
Existence of an anchor is enforced by the Java type-checker (ADR-0001): a `@Realizes` or `@Verifies` reference to a specification that no longer exists is a symbol that no longer compiles.
The verifiable symbols come from the `clew`-generated traceables; the engine borrows the checker rather than reimplementing a check of its own.
...
Invariants — no item is ever double-booked, a confirmation is atomic, a user's active count never exceeds quota — are CON specs, verified by tests anchored with `@Verifies`.
The type-checker proves the test is bound to the constraint; the test proves the behaviour.
...
Like the other structural rules, this is pinned by an ARCH spec authored when the packages it governs are built, not by an ADR.
...
It does not talk to a spec-authoring agent, to clew, or to `javac` — those are construction-time concerns and belong in "how this system is built", not in a view of what the running system interacts with.
...
A rejected operation — an unavailable item, an expired hold, a non-owner cancel or release, an over-quota claim, an invalid hold set — is signalled by throwing a **dedicated unchecked exception** named for the reason (fail-fast, `003` §2), never an in-band empty or `false` result.
...
They are not the checked truth; the ARCH and CON specs and the ADRs are.
