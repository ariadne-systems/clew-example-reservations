...
This contract states the runtime and the libraries; the architecture is stated in `docs/spec/architecture.md`.
...
clew itself runs on Node and is invoked through pnpm.
That is construction-time tooling, not part of this stack: nothing the engine ships depends on Node.
...
The traceables clew emits are **generated sources**, produced by `clew spec` and compiled with the project.
Production and test code anchor to them with the generator's trace markers, whose exact form is documented in the generated traceables' own README — that README is the single source for the marker spelling, not this contract (ADR-0001).
Generated traceables are never hand-edited.
