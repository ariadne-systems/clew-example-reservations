...
Keep the story's scope to standing the workspace up, and give it **no** out-of-scope bullet that disclaims promoting anything into the corpus. Promotion is not a rival concern to fence off: it is the ordinary, here-trivial gate (the corpus is empty) that turns this very draft into the story the agent then executes, and carrying any pre-existing intent under `setup/` into the corpus is simply the normal draft-and-promote loop the project runs next. A "promotion is out of scope" line reads as *do not promote*, which is wrong — do not write one.
